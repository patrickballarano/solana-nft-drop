...
const CandyMachine = ({ walletAddress }) => {
    ...
    return (
        <div className="machine-container">
            <p>{candyMachine ? `Drop Date: ${candyMachine.state.goLiveDateTimeString}` : 'Drop Date: n/a'}</p>
            <p>{candyMachine ? `Items Minted: ${candyMachine.state.itemsRedeemed} / ${candyMachine.state.itemsAvailable}` : 'Items Minted: 0'}</p>
            <button className="cta-button-2 mint-button" onClick={mintToken}>
                Mint NFT
            </button>
        </div>
    );
